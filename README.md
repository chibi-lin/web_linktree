...
## 概述
這是個人作品集網站原始碼，用於展示插畫和動畫委託服務。網站使用 **HTML**、**CSS** 和 **JavaScript** 構建，託管於 **GitHub Pages**，並透過 **Cloudflare** 管理自定義域名及 SSL/TLS 設置。
...
- **CNAME**：包含自定義域名，用於 GitHub Pages 配置。
...
4. **部署到 GitHub Pages**：
   ...
   - 添加 `CNAME` 檔案，內容為自定義域名。
   - 推送更改至 `main` 分支以觸發部署。
5. **配置自定義域名**：
   - 在 GitHub Pages 設置中，將自定義域名設置。
   - 在 Cloudflare 中配置 DNS 記錄：
     - A 記錄自定義域名指向 GitHub Pages 的 IP 地址：
       ```
       185.199.108.153
       185.199.109.153
       185.199.110.153
       185.199.111.153
       ```
     - CNAME 記錄自定義域名指向 `chibi-lin.github.io`。
...
## 注意事項
- **HTTPS 問題**：若 HTTPS 不可用，請確認 Cloudflare 中自定義域名的代理狀態為 **僅 DNS**，並確保 SSL/TLS 模式為 **完全（Full）**。等待 GitHub Pages 生成 Let’s Encrypt 證書（可能需數小時）。
- **委託條款**：網站詳細列出委託規則，例如動畫不接受 NSFW 內容、不接受急件、特定檔案格式（例如 .PSD、.MP4）。
- **自定義域名**：網站使用自定義域名，DNS 透過 Cloudflare 管理。確保 DNS 記錄正確配置，以避免 HTTPS 或網站訪問問題。
